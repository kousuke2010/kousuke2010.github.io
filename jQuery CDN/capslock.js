/**
* Caps	Lock
* @since for example：$("#test").capsLockTip({width:"200px",text:"大写状态开启"});
*/
(function($) {
	var curID = 0;
	$.fn.extend({
	...
	$.CapsLockTip = function(input,options) {
		...
		var $div = $("#autocapsLock" + $input.attr("curCapsLockId")).length != 0 ? $("#autocapsLock"+ $input.attr("curCapsLockId"))
				: $("<div/>").attr("id","autocapsLock" + $input.attr("curCapsLockId"))
				.css({
					"padding" : "0px 2px",
					"position" : "absolute",
					"background":"#fff",
				...
				.css("width",width)
				.css({"top":$input.offset().top + $input.height() + 3 + "px","left":$input.offset().left + "px"})
				.appendTo("body").text(options.text).hide();
		if(!$div.attr("class")&&options.styleClass){
			$div.addClass(options.styleClass);
		}else{
			$div.css({color:"#ff6600","font-size":"12px",border:"solid 1px #DBC492","border-bottom-color":"#B49366","border-right-color":"#B49366"});
		}
